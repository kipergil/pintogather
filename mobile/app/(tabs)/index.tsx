          ...
          <View className="max-h-[75%] rounded-t-3xl bg-white p-6">
            <View className="mb-4 flex-row items-center justify-between">
              ...
              <Pressable
                onPress={onClose}
                hitSlop={8}
                testID="button-close-folder-manager"
              >
        ...
        <View className="max-h-[75%] rounded-t-3xl bg-white p-6">
          <View className="mb-4 flex-row items-center justify-between">
            ...
            <Pressable
              onPress={onClose}
              hitSlop={8}
              testID="button-close-move-to-folder"
            >
              <Ionicons name="close" size={22} color="#64748b" />
            </Pressable>
          </View>
          <ScrollView>
            ...
            {flatFolders.length === 0 && (
              <Text className="text-sm italic text-slate-400">
                No folders yet — create one from the folder icon above.
              </Text>
            )}
          </ScrollView>
        </View>
...
export default function MapsListScreen() {
  ...
  const visibleMaps =
    showArchived || selectedFolderId === undefined
      ? (maps ?? [])
      : (maps ?? []).filter(
          (map) => (map.folderId ?? null) === selectedFolderId,
        );

  const onRestore = (map: MapListItem) => {
    Alert.alert(
      "Restore map?",
      `"${map.name}" will reappear in your maps list.`,
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Restore",
    ...
    <Screen>
      <View className="flex-row items-center justify-between py-4">
        <Text className="text-2xl font-bold text-slate-900">
          {showArchived ? "Archived maps" : "My Maps"}
        </Text>
        <View className="flex-row items-center gap-3">
          {!showArchived && (
            <Pressable
      ...
      {!showArchived && (maps ?? []).length > 0 && (
        ...
        >
          {[
            ...
            ...flatFolders.map((f) => ({
              id: f.id as string | null | undefined,
              label: f.name,
              testId: f.id,
            })),
          ].map((chip, i) => (
            ...
            >
              <Text
                className={`text-sm font-medium ${selectedFolderId === chip.id ? "text-primary" : "text-slate-600"}`}
              >
                {chip.label}
              </Text>
            </Pressable>
      ...
      <FlatList
        ...
        renderItem={({ item }) =>
          showArchived ? (
            <MapCard
              map={item}
              onPress={() => router.push(`/map/${item.shareUrl}`)}
              archived
          ...
          !isLoading ? (
            showArchived ? (
              <EmptyState
                ...
                description="Maps you archive will show up here."
              />
            ) : (maps ?? []).length > 0 ? (
              <EmptyState
                icon="folder-outline"
                title="No maps in this folder"
                description="Move a map here, or pick a different folder above."
              />
            ) : (
              <EmptyState
                icon="map-outline"
                title="No maps yet"
                description="Create your first map to start pinning places."
              >
                <Pressable
                  onPress={() => router.push("/map/create")}
                  className="mt-2"
      ...
      />
      <MoveToFolderSheet
        visible={!!moveMapTarget}
        onClose={() => setMoveMapTarget(null)}
        folders={folders}
        map={moveMapTarget}
      />
    </Screen>
