...
export default function PublicProfileScreen() {
  ...
  const displayName =
    profile?.fullName || (profile ? `@${profile.username}` : "");
  const totalPins =
    profile?.maps.reduce((sum, map) => sum + map.pinCount, 0) ?? 0;
  ...
  const socialLinks = profile
    ? ([
        [
          "twitter",
          buildSocialUrl("twitter", profile.twitterHandle),
          "logo-twitter",
        ],
        [
          "instagram",
          buildSocialUrl("instagram", profile.instagramHandle),
          "logo-instagram",
        ],
        [
          "linkedin",
          buildSocialUrl("linkedin", profile.linkedinHandle),
          "logo-linkedin",
        ],
      ] as const)
    : [];
  ...
  return (
    <View className="flex-1 bg-white">
      <Stack.Screen
        options={{ title: profile ? `@${profile.username}` : "Profile" }}
      />
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="This username isn't taken, or the link is no longer valid."
        />
      ) : (
        <FlatList
          className="px-4"
          ListHeaderComponent={
            <View className="gap-3 py-5">
              ...
              {profile.bio && (
                <Text className="text-sm text-slate-700">{profile.bio}</Text>
              )}

              <View className="flex-row flex-wrap items-center gap-x-4 gap-y-1">
                <Text className="text-sm text-slate-500">
                  {profile.maps.length}{" "}
                  {profile.maps.length === 1 ? "map" : "maps"}
                </Text>
                <Text className="text-sm text-slate-500">
          ...
          renderItem={({ item: map }) => (
            <Link href={`/map/${map.shareUrl}`} asChild>
              <Pressable
                ...
                testID={`link-profile-map-${map.id}`}
              >
                <Text
