...
const envSchema = z.object({
  ...
  APP_NAME: z.string().min(1).default("PinTogather"),
  ...
  APP_BASE_URL: z.string().url().default("https://pintogather.vercel.app"),
  /** Email for the dedicated, non-human directus_users row the Express server's static DIRECTUS_SERVICE_TOKEN lives on. */
  SERVICE_ACCOUNT_EMAIL: z.string().email().default("service@pintogather.dev"),
});
