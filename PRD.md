...
**Curation is admin-only and Directus-managed** — no custom admin screen was built for it. All five curated fields are edited directly on the map's row in Directus (reachable from the app via the existing "Open in Directus" quick-edit button, the same pattern already used for other admin-only fields), matching how `is_admin`/`user_group` are managed elsewhere in this app.

**Visibility gating** (`TIER_LIMITS.maxCuratedMapsVisible`): freemium and anonymous visitors see only the top `curatedOrder` maps (3); Basic/Premium see everything. `GET /api/discover` (public, unauthenticated) applies this cap server-side and returns `totalCount`/`visibleCount`/`isLimited` so the page can show an upgrade banner rather than silently truncating. Filter facets (`filters.categories`/`countries`/`citiesByCountry`) are derived from whatever is *actually* curated at request time, not the full fixed enum — so the UI never offers a filter that dead-ends on zero results as the catalog grows.
...
Clicking a Discover card goes to the map's normal `/map/:shareUrl` page — curated maps are real, live maps like any other, not a static showcase, so they can keep growing via ordinary contributions.
