...
|------|-------|----------|--------|
| **Freemium** | Free | 3 maps, 50 pins/map, 3 AI generations/day (prompt or image), 2 collaborators/map, 3 curated maps on /discover | AI generation + map/pin/seat limits enforced |
| **Basic** | £4.99/month | 10 maps, 200 pins/map, 15 AI generations/day (prompt or image), 8 collaborators/map, archive & restore maps, custom pin colors & icons, full /discover access | AI generation + map/pin/seat limits + map archiving + pin customization gates enforced |
| **Premium** | £9.99/month | Unlimited maps/pins/seats, 200 AI generations/day (prompt or image), custom branding, archive & restore maps, custom pin colors & icons, full /discover access | AI generation + map/pin/seat limits + custom branding + map archiving + pin customization gates enforced |
...
**The add-items hub (`/map/:shareUrl/add`).** The single place items enter a collection, replacing the old `/map/:shareUrl/import` page (that path still redirects, so existing links keep working). It is deliberately prominent rather than menu-buried — bulk and AI adding are the product's main draw, so map-detail carries a primary "Add pins/items" button, the map toolbar carries a "Bulk add with AI" shortcut, an empty collection shows method cards in place of the empty table, and creating a collection lands here (`?new=1`) instead of on an empty map. `?method=file|paste|ai|venue|map` deep-links a tab.

Internally every input method produces the same `StagedItem`, resolved by a per-type strategy — `location` via Google Places, `link` via `POST /api/link-preview`, `recommendation` needs no lookup — feeding one review list and one `POST /api/maps/:shareUrl/pins/bulk`. That is what makes each method a parser rather than an integration. Method tabs stay on screen once items are staged and staging *appends*, because venue search and drop-a-pin are inherently one-at-a-time; a batch can therefore mix pasted, AI-extracted, searched, and map-dropped items before being saved together. `SimpleGoogleMap`'s `onStageLocation` prop is what lets the embedded map stage a confirmed click instead of opening the add-pin dialog and saving on its own.

**Image-based AI extraction.** Any tab that can take an image uses the shared `ImageDropzone`/`useImagePaste` (`client/src/components/image-dropzone.tsx`): clipboard paste, drag-and-drop, or a multi-select picker, up to 4 images of PNG/JPEG/WebP/GIF at 4MB each. Clipboard paste matters most — screenshots usually live on the clipboard, never on disk — and the handler only swallows the paste event once an image is actually present, so pasting text into a textarea on the same page still works. `POST /api/maps/:shareUrl/extract-items` (multipart, `isAuthenticated`; the old `/venue-suggestions/from-screenshot` path is kept as an alias) accepts up to 4 images, uploads each to Directus via `storage.uploadVenueScreenshot` (`<userId>/uploads/`), and sends them as vision content blocks to Claude. Both this and the text route pick their system prompt from the collection's own `itemType` (`EXTRACT_SYSTEM_PROMPTS`), so a link collection gets pages and a recommendation collection gets things rather than venue names for everything, and both return `{ name, url?, note? }` objects (bare strings still parsed for back-compat) so link and recommendation imports carry real content. One request costs one AI generation regardless of image count. `screenshotImport` is **on for every tier including freemium** — the daily `aiSuggestionsPerDay` budget is what limits cost, and tiers differentiate on that quota rather than on access.
...
**Curation is admin-only**, via an in-app flow (not raw Directus editing): the Admin panel (`/admin`) has a "Map Curation" section listing every map across every owner (`GET /api/admin/maps`, name/owner search filter client-side), each with a "Curate"/"Edit curation" button leading to `/admin/maps/:mapId/curate` — a dedicated edit page (`admin-curate-map.tsx`) with a Curated toggle, category/country/city selects (the same cascading country→city narrowing as the Discover filter, sourced from `CURATED_CITY_BY_COUNTRY`), a display-order number field, and a tagline textarea. Saving goes through `PUT /api/admin/maps/:mapId/curate` (admin-gated, validated by `curateMapSchema` — requires category+country+city when turning curation on, and checks the city actually belongs to the selected country), which never touches the map's `owner`, so converting someone else's map into a curated one keeps them credited. Un-curating (toggling off) leaves the taxonomy fields in place rather than clearing them, so re-curating later doesn't mean re-entering everything. `curated_city`'s Directus field still carries the same cascading `meta.conditions` (§ above) for anyone who does reach for direct Directus editing, but it's no longer the primary path.

**Visibility gating** (`TIER_LIMITS.maxCuratedMapsVisible`): freemium and anonymous visitors get every curated map back from `GET /api/discover` (so the page can show what exists as a teaser) but only the top `curatedOrder` maps (3) come with a `shareUrl` — the rest are marked `locked: true` with `shareUrl: null`, so the client can render them (title/category/tagline/pin count) without being able to link anywhere but `/pricing`, enforced server-side rather than just hidden client-side. Basic/Premium get every map fully unlocked. The Discover page renders locked cards grayscale with a lock badge and an "Upgrade to see" hover overlay. `totalCount`/`visibleCount`/`isLimited` still drive the upgrade banner. Filter facets (`filters.categories`/`countries`/`citiesByCountry`) are derived from whatever is *actually* curated at request time, not the full fixed enum — so the UI never offers a filter that dead-ends on zero results as the catalog grows.

**Cover images.** Each Discover card gets an auto-generated 640×400 branded cover (`client/src/lib/discover-cover.ts`) — gradient colored per category (`CURATED_CATEGORY_GRADIENT`), a deterministic decorative pin-glyph scatter seeded from the map id, and the map's title. Deliberately no real map screenshot: that would need the Google Maps Static API, a separate billable API from the Maps JavaScript API this app already uses, skipped by design (mirrors the same call made for the share-image feature in §3.4). The canvas-drawing helpers (seeded PRNG, pin glyph, title auto-wrap/shrink) are shared with `share-image.ts` via `client/src/lib/canvas-card.ts` rather than duplicated.

Clicking an unlocked Discover card goes to the map's normal `/map/:shareUrl` page — curated maps are real, live maps like any other, not a static showcase, so they can keep growing via ordinary contributions. The header's "Discover" nav link is always visible (icon-only below the `sm` breakpoint, icon+label above it).
